...
const CommutePage: ExtendedNextPage = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    e.preventDefault();
    try {
      const { data } = await apiCall("commute", {
    ...
    <div className="max-w-7xl w-full mx-auto">
      <div className="flex justify-center items-center w-full h-60 bg-primary-400">
        <h1 className="text-center text-white text-xl sm:text-3xl tracking-wide font-semibold">
          Carbon Emission from your Commute
        </h1>
      </div>
      ...
      <div className="mt-16 w-full">
        <form onSubmit={handleSubmit}>
          ...
          <div className="w-full flex-col sm:flex-row mt-6 sm:mt-8 flex space-y-8 sm:space-y-0 sm:space-x-8 justify-center items-center mx-auto">
            ...
            <button
              disabled={loading}
              className="max-w-sm w-full rounded-lg bg-primary-200 text-white font-semibold text-lg py-1.5"
            >
      ...
      {!!emission && <EmissionCard emission={emission} type="commute" />}
    </div>
  );
};
CommutePage.Layout = BaseLayout;
